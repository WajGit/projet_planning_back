...
#[Route('/api/week')]
final class WeekTypeController extends AbstractController
{
    ...
    public function add(Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        if (empty($data['name']) || empty($data['planningId'])) {
            return new JsonResponse(['error' => 'Nom ou planning manquant'], 400);
        }

        $planning = $entityManager->getRepository(PlanningType::class)->find($data['planningId']);
        if (!$planning) return new JsonResponse(['error' => 'Planning introuvable'], 404);
    ...
    public function update(int $id, Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        $week = $entityManager->getRepository(WeekType::class)->find($id);
